...
const TechnologyService = {
  ...
  getPlayerEffectiveTechnologyLevels(
    game: Game,
    player: Player | null,
    sanitize: boolean = true
  ): PlayerTechnologyLevels {
  ...
  getStarEffectiveTechnologyLevels(
    ...
    let player = star.ownedByPlayerId
      ? game.galaxy.players.find((x) => x.id === star.ownedByPlayerId) || null
      : null;
    let techs = this.getPlayerEffectiveTechnologyLevels(game, player, false);

    if (star.specialistId) {
      let specialist = SpecialistService.getByIdStar(star.specialistId);
  ...
  getCarrierEffectiveTechnologyLevels(
    ...
    let player =
      game.galaxy.players.find((x) => x.id === carrier.ownedByPlayerId) || null;
    let techs = this.getPlayerEffectiveTechnologyLevels(game, player, false);

    // Apply any specialist tech modifiers.
    if (carrier.specialistId) {
      let specialist = SpecialistService.getByIdCarrier(carrier.specialistId);
    ...
    if (carrier.specialistId) {
      ...
      if (specialist.modifiers.local) {
        if (
          ...
          if (
            isAttacker &&
            specialist.modifiers.local.carrierToStarCombat.attacker?.weapons
          ) {
            buffs.push(
              specialist.modifiers.local.carrierToStarCombat.attacker.weapons
            );
          ...
          if (
            !isAttacker &&
            specialist.modifiers.local.carrierToStarCombat.defender?.weapons
          ) {
            buffs.push(
              specialist.modifiers.local.carrierToStarCombat.defender.weapons
            );
  ...
  getCarriersWeaponsDebuff(carriersToCheck: Carrier[]) {
    ...
    deduction = carriersToCheck
      ...
      .map((c) => {
        let specialist = SpecialistService.getByIdCarrier(c.specialistId!);
  ...
  getStarEffectiveWeaponsLevel(
    game: Game,
    defenders: Player[],
    star: Star,
    carriersInOrbit: Carrier[]
  ) {
  ...
  getCarriersEffectiveWeaponsLevel(
    game: Game,
    players: Player[],
    carriers: Carrier[],
    isCarrierToStarCombat: boolean,
    ...
    let buffs = carriers.map((c) =>
      this.getCarrierWeaponsBuff(
        c,
        isCarrierToStarCombat,
        isAttacker,
        players.length
