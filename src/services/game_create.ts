...
const GameCreateService = {
  create(settings: GameSettings) {
    ...
    if (desiredPlayerStarCount > desiredStarCount) {
        throw new Error(`Cannot create a galaxy of ${desiredStarCount} stars with ${game.settings.player.startingStars} stars per player.`);
    }
...
_setGalaxyCenter(game: Game) {
    ...
    game.constants.distances.galaxyCenterLocation = MapService.getGalaxyCenter(starLocations);
},
...
_setupTutorialPlayers(game: Game) {
    ...
    GameJoinService.assignPlayerToUser(game, game.galaxy.players[0], game.settings.general.createdByUserId!, `Player`, 0);
