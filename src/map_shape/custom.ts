...
const CustomMapService = {
  generateLocations(customJSON: string, playerLimit: number): Location[] {
    ...
    for (const star of json.stars) {
      ...
      star.isAsteroidField =
        star.isAsteroidField == null ? false : star.isAsteroidField;
      ...
      star.wormHoleToStarId =
        star.wormHoleToStarId == null ? null : +star.wormHoleToStarId;
    ...
    if (homeStars.length !== playerLimit) {
      throw new Error(
        `Must have ${playerLimit} capital stars in the custom map.`
      );
    }
    ...
    if (homeStars.length === playerIds.length) {
      ...
      throw new Error(
        "Unequal amount of home stars and players, or repeated player IDs"
      );
    ...
    for (let loc of locations.filter((l) => l.wormHoleToStarId != null)) {
      loc.wormHoleToStarId = locations.find(
        (l) => l.id === loc.wormHoleToStarId
      )?._id;

      if (
        !loc.wormHoleToStarId ||
        loc.wormHoleToStarId.toString() === loc._id.toString()
      ) {
        throw new Error(
          `Worm hole to star id is invalid for ${JSON.stringify(loc)}`
        );
      }
    }
  ...
  _checkStarProperty(
    ...
    if (star === undefined) throw new Error(`Missing property of star ${star}`);
    if (star?.[property] === undefined)
      throw new Error(
        `Missing property ${property} of star ${JSON.stringify(star)}`
      );
    ...
    if (typeof star[property] !== type)
      throw new Error(
        `Invalid type property ${property} of star ${JSON.stringify(star)}`
      );
